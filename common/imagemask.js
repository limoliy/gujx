...
//###ADDED by Jimmy 2018/8/28
/**
 * 小程序化一个 context, 使原用于HTML5的 Canvas Context 适用于微信小程序
 * @param ctx
 * @returns {*}
 */
ImageMask.prototype._wxapp_lize = function(ctx){
    if(!ctx._is_wxapp_lized) {
        ctx._is_wxapp_lized = true;
        //
        ctx.canvas = this.opts.canvas;
        ...
        ctx.getImageData = (x, y, width, height) => {
            return new Promise((resolve, reject) => {
                width = width || this.opts.canvas.width;
                height = height || this.opts.canvas.height;
                wx.canvasGetImageData({
                    canvasId: this.opts.canvas.id,
                    x: x, y: y,
                    width: width, height: height,
                    success: (res) => {
                        resolve({code: 1, data: res.data, width: res.width, height: res.height});
                    },
                    fail: (err) => {
                        reject({code: 0, data: null, msg: err})
                    }
                });
            })
        };
            ...
            return new Promise((resolve, reject) => {
                ...
                wx.canvasPutImageData({
                    ...
                    success: (res) => {
                        ...
                        resolve({code: 1, msg: res, width: width, height: height});
                    },
                    fail: (err) => {
                        console.log('绘制出错了' + err);
...
//###END
ImageMask.prototype.hideText = async function(canvas, text){
    //###ADDED by Jimmy 2018/8/28
    canvas = this._wxapp_lize(canvas);
    //###END
    ...
    var imgData = await ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height);
    ...
    return imgData;
    // return await ctx.putImageData(imgData, 0, 0, ctx.canvas.width, ctx.canvas.height);
};

ImageMask.prototype.revealText = async function(canvas){
    //###ADDED by Jimmy 2018/8/28
    canvas = this._wxapp_lize(canvas);
    //###END
    var ctx = canvas.getContext('2d');
